...
def solution(input_data, d, no_elves_die):
    ...
    while True:
        ...
        for idx, c in enumerate(order):
            ...
            if enemy_positions_in_range:
                ...
                lowest_health = min(
                    enemies, key=lambda e: (e.hp, reading_order(e.pos()))
                )
                ...
                if (
                    no_elves_die
                    and lowest_health.unit_type == "E"
                    and not lowest_health.is_alive
                ):
                    return False, 0
...
def part1(input):
    _, score = solution(input, 3, False)
...
def part2(input):
    i = 3
    while True:
        no_elves_died, score = solution(input, i, True)
        if no_elves_died:
            return score
        i += 1
